...
****
capture mkdir "`c(sysdir_site)'/raw/InformesTrimestrales/"
...
forvalues k = 2019(1)`=aniovp' {
	foreach j in i ii iii iv {
		...
		capture confirm file "`c(sysdir_site)'/raw/InformesTrimestrales/Actividad_`k'_`j'.dta"
		if _rc == 0 continue
		...
		* Limpia *
		...
		save "`c(sysdir_site)'/raw/InformesTrimestrales/Actividad_`k'_`j'.dta", replace
	}
}
forvalues k = 2019(1)`=aniovp' {
	foreach j in i ii iii iv {
		noisily di `k' "`j'"
		if "`first'" == "" {
			use "`c(sysdir_site)'/raw/InformesTrimestrales/Actividad_`k'_`j'.dta", clear
			local first "first"
		}
		else {
			capture append using "`c(sysdir_site)'/raw/InformesTrimestrales/Actividad_`k'_`j'.dta"
		}
	}
}
...
forvalues k = 2019(1)2025 {
	foreach j in i ii iii iv {
		...
		capture mkdir "`c(sysdir_site)'/raw/InformesTrimestrales/"
		save "`c(sysdir_site)'/raw/InformesTrimestrales/`k'_`j'_`l'", replace
	}
}
forvalues k = 2019(1)2025 {
	foreach j in i ii iii iv {

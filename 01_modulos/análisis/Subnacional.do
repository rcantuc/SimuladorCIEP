...
*** 1. PIB Estatal ***
**********************
** 1.1 PIBE (PIB por entidad federativa, a precios corrientes, base 2018) **
AccesoBIE 760461 760503 760545 760587 760629 760671 760713 760755 760797 760839 ///
	760881 760923 760965 761007 761049 761091 761133 761175 761217 761259 ///
	761301 761343 761385 761427 761469 761511 761553 761595 761637 761679 ///
	761721 761763, ///
	nombres(pibYEntAgs pibYEntBC pibYEntBCS pibYEntCamp pibYEntCoah pibYEntCol ///
	pibYEntChis pibYEntChih pibYEntCDMX pibYEntDgo pibYEntGto pibYEntGro ///
	pibYEntHgo pibYEntJal pibYEntEdoMex pibYEntMich pibYEntMor pibYEntNay ///
	pibYEntNL pibYEntOax pibYEntPue pibYEntQro pibYEntQRoo pibYEntSLP ///
	pibYEntSin pibYEntSon pibYEntTab pibYEntTamps pibYEntTlax pibYEntVer ///
	pibYEntYuc pibYEntZac)

foreach k of varlist pibYEnt* {
	replace `k' = `k' * 1000000
}
...
** 1.3 ITAEE (variación anual, total de la actividad económica estatal) **
AccesoBIE 741218 741219 741220 741221 741223 741225 741226 741228 741229 741230 ///
	741231 741232 741233 741234 741235 741236 741237 741238 741239 741241 ///
	741242 741244 741245 741246 741248 741249 741250 741252 741254 741255 ///
	741257 741258, ///
	nombres(ITAEEAgs ITAEEBC ITAEEBCS ITAEECamp ITAEECoah ITAEECol ///
	ITAEEChis ITAEEChih ITAEECDMX ITAEEDgo ITAEEGto ITAEEGro ///
	ITAEEHgo ITAEEJal ITAEEEdoMex ITAEEMich ITAEEMor ITAEENay ///
	ITAEENL ITAEEOax ITAEEPue ITAEEQro ITAEEQRoo ITAEESLP ///
	ITAEESin ITAEESon ITAEETab ITAEETamps ITAEETlax ITAEEVer ///
	ITAEEYuc ITAEEZac)

collapse (mean) ITAEE*, by(anio)

local anioultimo = anio[_N]
local aniosiguiente = `anioultimo' + 1
set obs `=_N+1'
replace anio = `aniosiguiente' in -1
foreach k of varlist ITAEE* {
	capture replace `k' = ${pib`aniosiguiente'} in -1
	if _rc != 0 {
		replace `k' = ${pib`anioultimo'} in -1
	}
}
...
***************************************************
import excel "`c(sysdir_site)'/raw/ENOE/Población ocupada.xlsx", clear
drop if B == ""
drop AI
...
***********************************
*capture mkdir "`c(sysdir_site)'/raw/Subnacional/estatal"
*cd "`c(sysdir_site)'/raw/Subnacional/estatal"
*unzipfile "https://www.inegi.org.mx/contenidos/programas/finanzas/datosabiertos/efipem_estatal_csv.zip", replace

*capture mkdir "`c(sysdir_site)'/raw/Subnacional/CDMX"
*cd "`c(sysdir_site)'/raw/Subnacional/CDMX"
*unzipfile "https://www.inegi.org.mx/contenidos/programas/finanzas/datosabiertos/efipem_cdmx_csv.zip", replace

forvalues anio=2007(1)`=anioPE' {

	***************
	** 4.1 Bases **
	** Bases de estados **
	import delimited "`c(sysdir_site)'/raw/Subnacional/estatal/conjunto_de_datos/efipem_estatal_anual_tr_cifra_`anio'.csv", encoding(UTF-8) clear
	tempfile estados
	save "`estados'"

	** Bases de la CDMX **
	import delimited "`c(sysdir_site)'/raw/Subnacional/CDMX/conjunto_de_datos/efipem_cdmx_anual_tr_cifra_`anio'.csv", encoding(UTF-8) clear
	...
	** 4.4 Comprobar que los valores están bien **
	...
	noisily di in g "  Año `anio': " in y "max |cuadrador| = " %20.1fc max(abs(cuadrador))
	restore
...
gen ent = ""
forvalues k=1(1)32 {
	replace ent = "``k''" if id_entidad == `k'
}


** Guardar **
*drop id_entidad
order anio id_entidad entidad ent capitulo concepto partida subpartida divCIEP valor
...
*** 5. Deuda ***
****************
import excel "`c(sysdir_site)'/raw/Subnacional/DEUDA SUBNACIONAL TOTAL.xlsx", clear cellrange(A2:BM42)
...
keep if anio >= 2003 & anio <= `=anioPE'
save "`c(sysdir_site)'/master/EstadosBaseINEGI.dta", replace
...
merge m:1 (anio entidad) using `poblacionOcupada', nogen
keep if anio >= 2003 & anio <= `=anioPE'
save "`c(sysdir_site)'/master/EstadosBaseEstOpor.dta", replace


/** 6.3 SICUENTAS **
use "`c(sysdir_site)'/raw/Otros/gasto_salud.dta", clear
drop if CVE_ENT == .
compress
rename _all, lower
...
save "`c(sysdir_site)'/master/GastoEstatalSalud.dta", replace


** 6.4 Seguridad **
use "`c(sysdir_site)'/raw/Otros/gasto_seguridad.dta", clear
compress
rename _all, lower
...
save "`c(sysdir_site)'/master/GastoEstatalSeguridad.dta", replace
...
*forvalues anio=1950(1)2050 {
	foreach entidad in $entidadesL {
		noisily Poblacion if entidad == "`entidad'", $update anio(`=anioPE') //aniofinal(2030)
	}
*}
...
********************************
capture use "`c(sysdir_site)'/master/EstadosBaseEstOpor.dta", clear
if _rc != 0 {
	noisily run "`c(sysdir_site)'/01_modules/analysis/Subnacional.do"
	use "`c(sysdir_site)'/master/EstadosBaseEstOpor.dta", clear
}

collapse (mean) pob* deflator pibYEnt, by(anio entidad entidadx)
sort entidadx anio

summarize anio if pibYEnt != ., meanonly
local aniolast = r(max)
...
foreach k of global entidadesC {
	...
	* Texto sobre gráficas *
	forvalues j=1(1)`=_N' {
		...
		if entidad[`j'] == "`k'" & montograph[`j'] != . & anio[`j'] == `aniolast' {
...
*****************************
use "`c(sysdir_site)'/master/EstadosBaseINEGI.dta", clear
keep if valor != .
rename valor monto
...
local aniolastGF = r(max)
forvalues k=1(1)`=_N' {
	if anio[`k'] == `aniolastGF' {
		if concept[`k'] == 1 {
...
replace monto = L.monto*crec_pib if monto == . & L.monto != . & crec_pib != .

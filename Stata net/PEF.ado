...
program define UpdatePEF
	...
	* 1.1. Descargar archivos *
	capture mkdir "`c(sysdir_site)'/raw/PEFs"
	cd "`c(sysdir_site)'/raw/PEFs"

	// CP 2013
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2013.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2013.dta", replace
	}

	// CP 2014
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2014.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2014.dta", replace
	}

	// CP 2015
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2015.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2015.dta", replace
	}

	// CP 2016
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2016.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2016.dta", replace
	}

	// CP 2017
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2017.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2017.dta", replace
	}

	// CP 2018
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2018.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2018.dta", replace
	}

	// CP 2019
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2019.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2019.dta", replace
	}

	// CP 2020
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2020.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2020.dta", replace
	}

	// CP 2021
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2021.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2021.dta", replace
	}

	// CP 2022
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2022.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2022.dta", replace
	}

	// CP 2023
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2023.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2023.dta", replace
	}

	// CP 2024
	capture confirm file "`c(sysdir_site)'/raw/PEFs/CP 2024.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/CP 2024.dta", replace
	}

	// PEF 2025
	capture confirm file "`c(sysdir_site)'/raw/PEFs/PEF 2025.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/PEF 2025.dta", replace
	}

	// PPEF 2026
	capture confirm file "`c(sysdir_site)'/raw/PEFs/PEF 2026.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/raw/PEFs/PEF 2026.dta", replace
	}
	...
	save "`c(sysdir_site)'/raw/PEFs/Cuotas ISSSTE.dta", replace

	capture confirm file "`c(sysdir_site)'/temp/prePEF.dta"
	if _rc != 0 {
		local archivos: dir "`c(sysdir_site)'/raw/PEFs" files "*.dta"		// Archivos .xlsx
		*local archivos `""PEF 2025.dta" "CuotasISSSTE.dta""'
		...
		foreach k of local archivos {
			...
			* 1.1 Importar el archivo `k'.xlsx (Cuenta Pública) *
			...
			tokenize `k'
			use "`c(sysdir_site)'/raw/PEFs/`k'", clear
			capture drop v*

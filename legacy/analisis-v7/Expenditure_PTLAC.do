...
timer on 15
	...
	if `1' >= 2022 & `1' < 2024 {
		...
		capture confirm file "`c(sysdir_site)'/raw/ENIGH/`anioenigh'/gastospersona.dta"
		if _rc != 0 {
			copy "https://www.dropbox.com/scl/fi/81oizm6udw7wgrrx4roj3/2022.zip?rlkey=z8evwh138uqz2e1no558kc1f0&dl=1" "`c(sysdir_site)'/raw/ENIGH/2022.zip", replace
	...
	if `1' >= 2020 & `1' < 2022 {
		...
		capture confirm file "`c(sysdir_site)'/raw/ENIGH/`anioenigh'/gastospersona.dta"
		if _rc != 0 {
			copy "https://www.dropbox.com/scl/fi/mj4gd8juoyl7pj2x780r0/2020.zip?rlkey=vvk8mnknngb5hmod7562bp4dl&dl=1" "`c(sysdir_site)'/raw/ENIGH/2020.zip", replace
	...
	if `1' >= 2018 & `1' < 2020 {
		...
		capture confirm file "`c(sysdir_site)'/raw/ENIGH/`anioenigh'/gastospersona.dta"
		if _rc != 0 {
			copy "https://www.dropbox.com/scl/fi/71edca9akpnpujdjmpovq/2018.zip?rlkey=nubev3sxg3vl37ubhgcx0tyb4&dl=1" "`c(sysdir_site)'/raw/ENIGH/2018.zip", replace
...
if _rc != 0 {
	...
	if _rc != 0 {
		...
		* Usar base auditada si existe (ENIGH 2024+), original si no
		capture confirm file "`c(sysdir_site)'/raw/ENIGH/`anioenigh'/clave_iva_auditada.dta"
		if _rc == 0 {
			merge m:1 (clave) using "`c(sysdir_site)'/raw/ENIGH/`anioenigh'/clave_iva_auditada.dta", ///
	...
	*  Mide qué fracción del precio final es valor creado en el sector.
	...
	if _rc != 0 {
		...
		gen double va_ratio = A131A / A111A
		replace va_ratio = . if A111A <= 0
		replace va_ratio = 0 if va_ratio < 0 & !missing(va_ratio)
		replace va_ratio = 1 if va_ratio > 1 & !missing(va_ratio)
	...
	** 2.8. Merge jerárquico: VA ratio → registros de gasto **
	...
	if _rc != 0 {
		* Construir lookup tables jerárquicos (6→5→4→3→2 dígitos)
		...
		forvalues n = 6(-1)2 {
			...
			bys code: egen double va_mean = mean(va_ratio)
		...
		forvalues k = 1/6 {
			...
			foreach n in 6 5 4 3 2 {
				tostring clase_de_actividad`k', replace force
				gen code = substr(clase_de_actividad`k', 1, `n')
				merge m:1 code using `LU`n'', keep(master match) nogen
				replace va`k' = va_mean if missing(va`k') & !missing(va_mean)
				drop code va_mean
			}
		}
	...
	egen proporcion = rowmean(va1 va2 va3 va4 va5 va6)

	summ proporcion [aw=factor]
	replace proporcion = r(mean) if missing(proporcion)

	* Mostrar proporción promedio ponderada
	noisily di in g "Proporción valor agregado último eslabón (VACB/PBT): " in y r(mean)
...
*merge 1:1 (folioviv foliohog numren) using "`c(sysdir_site)'/raw/CONEVAL/2022/Base final/pobreza_20.dta", nogen keepus(decil)

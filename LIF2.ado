...
quietly {
	...
	****************
	...
	if "$graphs" == "on" | "`graphs'" == "graphs" {
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & divCIEP != `deuda' & divCIEP == 16, ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:IVA observado y estimado}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosIVA, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & divCIEP == 8, ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:IEPS no petrolero observado y estimado}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosIEPSNP, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & divCIEP == 9, ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:IEPS petrolero observado y estimado}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosIEPSP, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & (divCIEP == 8 | divCIEP == 9) , ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:IEPS total observado y estimado}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosIEPST, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & divCIEP == 1, ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:Aprovechamientos observados y estimados}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosaprov, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & (divCIEP == 12 | divCIEP == 14) , ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:ISR PF observado y estimado}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosISRPF, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & divCIEP == 13, ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:ISR PM observados y estimados}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosisrpm, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		...
		graph bar (sum) LIFPIB recaudacionPIB if anio >= 2010 & (divCIEP == 12 | divCIEP == 14 | divCIEP == 13) , ///
			over(divCIEP, relabel(1 "LIF" 2 "Obs")) ///
			over(anio, label(labgap(vsmall))) ///
			stack asyvars ///
			title("{bf:ISR total observado y estimado}") ///
			ytitle(% PIB) ylabel(0(5)30, labsize(small)) ///
			legend(on position(6) rows(1)) ///
			name(ingresosISRT, replace) ///
			blabel(bar, format(%7.1fc)) ///
			caption("{it:Fuente: Elaborado por el CIEP, con informaci{c o'}n de la SHCP (Datos Abiertos y Paquetes Econ{c o'}micos).}")
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)
		gr_edit .plotregion1.GraphEdit, cmd(_set_rotate)

...
if _rc != 0 {
	...
	*  Mide qué fracción del precio final es valor creado en el sector.
	capture confirm file "`c(sysdir_site)'/03_temp/`anioenigh'/va_por_clase_actividad.dta"
	if _rc != 0 {
		...
		save "`c(sysdir_site)'/03_temp/`anioenigh'/va_por_clase_actividad.dta", replace
	}
	...
	** 2.8. Merge jerárquico: VA ratio → registros de gasto **
	...
	if _rc != 0 {
		* Construir lookup tables jerárquicos (6→5→4→3→2 dígitos)
		use "`c(sysdir_site)'/03_temp/`anioenigh'/va_por_clase_actividad.dta", clear

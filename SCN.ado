...
quietly {
	...
	** D.1. Cuenta de generaci{c o'}n del ingreso **
	...
	if _rc != 0 | "`update'" == "update" {
		...
		* Importa los datos para cada a駉, si hay un error, el loop termina *
	...
	* guarda el primer a駉 para el que hay un valor de PIB*
	...
	/* Calcula los valores futuros de las variables a partir de la 鷏tima 
	observaci髇 y los valores anteriores de 2002 a 1993 (utiliza la tasa de 
	crecimiento del PIB en t閞minos reales */
	...
	** C.1. Ingreso mixto **
	g double MixL = IngMixto*2/3			// NTA metodolog铆a
	format MixL %20.0fc
	label var MixL "Ingreso mixto (laboral)"

	g double MixK = IngMixto*1/3			// NTA metodolog铆a
	format MixK %20.0fc
	...
	** R.5 Display (de la producci贸n al ingreso) ***
	...
	** R.3. Graph **
	if "$graphs" == "on" | "`graphs'" == "graphs" {
		...
		twoway (area `AhorroN' `ConGob' `ConHog' `ComprasN' anio if anio <= `latest') ///
			...
			title("{bf:Utilizaci贸n del ingreso}") ///
			caption("{it:Fuente: Elaborado por el CIEP con el Simulador v5.}") ///
	...
	* Returns *
	...
	noisily di in g "  (+) Informaci贸n en medios masivos" ///
	...
	noisily di in g "  (+) Direcci贸n de corporativos y empresas" ///

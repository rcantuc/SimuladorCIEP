...
quietly {
	...
	* Output: `varlist'PC.dta *
	postfile PC2 double(estimacion contribuyentes poblacion montopc hombrespc mujerespc pob0_18pc pob19_64pc pob65pc) ///
		using `"${SIMROOT}/users/$id/bootstraps/`bootstrap'/`varlist'PC2"', replace
	...
	*** 2 Monto per capita ***
	**************************
	use `"${SIMROOT}/users/$id/bootstraps/`bootstrap'/`varlist'PC2"', clear
	...
	* Guardar en Excel *
	if "`excel'" == "excel" {
		local j = 12
		putexcel set "${SIMROOT}/users/$id/Deciles.xlsx", modify sheet("`varlist'")
	...
	******************************
	if "$nographs" != "nographs" & "`nographs'" != "nographs" {
		...
		graph export `"${SIMROOT}/users/$id/graphs/`varlist'_`aniope'.png"', replace name(`=substr("`varlist'",1,10)'_`aniope')

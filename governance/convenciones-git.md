...
Por eso existe `governance/scripts/verify_gitignore.sh`: define un conjunto de referencia de paths que **deben** estar ignorados (logs Stata, archivos de Dropbox conflicted copy, llaves `.key`, binarios de `raw/`) y paths que **no deben** estar ignorados (código del Simulador, documentos de governance, el `manifest.json` del Catálogo de datos asociados), y reporta cualquier divergencia.

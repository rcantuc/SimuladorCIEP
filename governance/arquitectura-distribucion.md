...
Las dos URLs son distintas (la del sub-canal HTML es el subdominio `simuladorfiscal.ciep.mx`; la del sub-canal Stata es la ruta `/simuladorfiscal` del dominio principal `ciep.mx`) y corren en infraestructura distinta: el sub-canal HTML está alojado en IONOS (VPS dedicado), el sub-canal Stata está alojado en Cloudways (mismo hosting que el sitio principal `ciep.mx` y sus micrositios). Ambos sirven al mismo motor de cálculo. El detalle de cada uno está en §6.4 (web HTML) y §6.5 (Stata `net from`).

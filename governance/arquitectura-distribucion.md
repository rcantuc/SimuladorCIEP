...
- **`publicar-endpoint.sh`** — script de deploy que lee el manifiesto, genera dinámicamente el `stata.toc` con la versión y fecha, hace backup remoto pre-deploy, sincroniza al servidor Cloudways vía `rsync` sobre SSH (con flag `--chmod` que produce permisos Apache compatibles — sin él los archivos sincronizados generan HTTP 403), verifica que el endpoint responde la versión correcta post-deploy, y registra el deploy en `governance/deploys/endpoint-stata.log`.
...
1. **Pisotea el working tree del clon Git.** Si el clon Git tiene HEAD apuntando a un commit distinto al `master` de desarrollo, rsync sobreescribe archivos sin que Git se entere, dejando el clon con working tree desincronizado de su HEAD. Detectable solo con `git status` posterior.
...
3. **No usa el mecanismo Git para versiones nuevas.** Si el clon Git ya tiene un tag local desactualizado, rsync no lo sincroniza al estado del tag remoto. El operador puede creer que sincronizó pero los tags del clon siguen viejos.
...
| ----------------------------------------------------------------------------------------------------------------| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Clon de desarrollo fuera de Dropbox                                                                            | Sigue dentro de Dropbox (`~/Library/CloudStorage/Dropbox-CIEP/.../SimuladorCIEP/`). Riesgo de corrupción del directorio `.git/` documentado en `governance/historico/fase-0-5-git-hygiene-audit.md`.                                                                                                 | Migración manual antes de Acción 6.                                                                                                                                               |

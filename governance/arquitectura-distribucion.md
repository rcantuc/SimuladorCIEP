...
Esa fricción motivó eliminar los `.ado` de `Stata net/` (commit `2daadf8`). El directorio ahora solo conserva los `.pkg` y `stata.toc` como placeholders del manifiesto de paquete. Esto deja la publicación al servidor `https://ciep.mx/simuladorfiscal/` **temporalmente sin proceso definido**: el staging manual ya no existe, pero el script automatizado que lo reemplazaría aún no se construyó.

**Limitaciones del flujo manual anterior** (registradas para futura referencia y evitar replicarlas en el reemplazo):
...
**Solución pendiente.** Reemplazar el staging manual eliminado por un script `publicar-endpoint.sh` que lea un manifiesto versionado (`manifest-endpoint.toml`) declarando qué archivos del repo forman parte del Simulador público vía `net from`, y sincronice al servidor con trazabilidad. Contrato funcional preliminar en §7. La construcción de este script y su manifiesto pasa de ser trabajo planeado para v8 a ser **pre-requisito para publicar cualquier versión nueva al endpoint público vía `net from`**. Mientras tanto, el endpoint sigue sirviendo la versión que ya tiene desplegada; no hay forma de actualizarla con la convención actual.
...
| **Script `publicar-endpoint.sh`**                                                                              | No implementado. El staging manual vía `Stata net/` fue eliminado en commit `2daadf8`; sin el script automatizado, la publicación al sub-canal Stata del endpoint público quedó temporalmente sin proceso definido. Estado actual documentado en §6.6.                                       | Pendiente — bloquea publicación de versiones nuevas al endpoint. Sin calendario fijo; requiere coordinación con Daniel Orduña sobre el rol de operación.                          |
| **Manifiesto `manifest-endpoint.toml`**                                                                        | No existe. Sería un archivo versionado en Git que declara explícitamente qué `.ado`, `.sthlp`, `.pkg`, `stata.toc` forman parte del Simulador "público vía `net from`" (subconjunto de todos los archivos del repo). Análogo al Catálogo de datos asociados pero para código de la Capa 4. | Pendiente, junto con `publicar-endpoint.sh`.                                                                                                                                      |
| **Borrado completo de `Stata net/`**                                                                           | Ejecutado parcialmente: los `.ado` se eliminaron en commit `2daadf8` (mayo 2026); los `.pkg` y `stata.toc` se conservan como placeholders del manifiesto de paquete. El borrado completo del directorio queda pendiente hasta que `publicar-endpoint.sh` o equivalente esté operativo y verificado. | Pendiente, después de validar el script automatizado.                                                                                                                             |

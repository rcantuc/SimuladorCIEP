...
`publicar.sh` es un wrapper de alto nivel que orquesta el flujo completo de publicación. Tiene tres sub-comandos según qué canal de distribución se quiere actualizar.

```bash
./publicar.sh release v8.0      # solo sub-canal Stata público
./publicar.sh internos v8.0     # solo Carpeta de investigadores
./publicar.sh todo v8.0         # ambos en orden: internos primero, release después
```

**Pre-condiciones comunes** (verificadas al inicio independientemente del sub-comando): estar en `master`, working tree limpio, alineación con `origin/master`. Si la etiqueta de versión no existe localmente, el script ofrece crearla en el HEAD actual mediante invocación interactiva de `$EDITOR` para escribir el mensaje del tag anotado (o vía flag `--tag-message="..."` para uso no-interactivo).

**Sub-comando `release` — publica al endpoint público `https://ciep.mx/simuladorfiscal/`**

| Elemento | Especificación |
|---|---|
| **Entrada** | Número de versión (ejemplo: `v8.0`). |
| **Qué hace** | (1) Verifica pre-condiciones comunes. (2) Push del tag a `origin` si aún no está. (3) Invoca `publicar-endpoint.sh` con la versión, que lee el manifiesto `manifest-endpoint.toml`, genera el `stata.toc` actualizado, hace backup remoto del endpoint vía `tar.gz`, sincroniza al servidor Cloudways vía `rsync` sobre SSH con `--chmod` para permisos Apache correctos, verifica post-deploy. |
| **Salida exitosa** | Endpoint `https://ciep.mx/simuladorfiscal/` sirve la versión nueva. Log appendado en `governance/deploys/endpoint-stata.log` con timestamp, SHA del commit, versión, operador, ruta del backup remoto. Mensaje en consola con URL del endpoint y comando de verificación (`net from https://ciep.mx/simuladorfiscal/` desde Stata externo). |
...
**Sub-comando `internos` — sincroniza al directorio de la Carpeta de investigadores**

| Elemento | Especificación |
|---|---|
| **Entrada** | Número de versión (ejemplo: `v8.0`). |
| **Qué hace** | (1) Verifica pre-condiciones comunes. (2) Push del tag a `origin` si aún no está. (3) Resuelve la ubicación de la Carpeta de investigadores: si `CARPETA_INVESTIGADORES_PATH` está definida en `endpoint-credentials.sh`, usa ese override; si no, evalúa candidatos típicos (`$HOME/Library/CloudStorage/Dropbox-CIEP/SimuladorCIEP` en Mac moderno, `$HOME/Dropbox-CIEP/SimuladorCIEP` en Linux o Mac clásico). (4) Crea backup tar.gz de la Carpeta en `_backups/` (con rotación de 5 backups). (5) Sincroniza el contenido del repo al directorio destino mediante `rsync` con lista de exclusiones (`.git/`, `governance/`, etc.). |
| **Salida exitosa** | Carpeta de investigadores con la versión nueva. Log en `governance/deploys/carpeta-investigadores.log`. |
| **Si falla** | El backup tar.gz pre-rsync permite recuperación. |
| **Modelo arquitectónico actual** | rsync con exclusiones. Modelo institucional correcto (clon Git con `git pull` directo) documentado en §6.7. Rediseño pendiente — ver §7. |

**Sub-comando `todo` — combina ambos**

| Elemento | Especificación |
|---|---|
| **Entrada** | Número de versión (ejemplo: `v8.0`). |
| **Qué hace** | Ejecuta `internos` primero (la Carpeta de investigadores tiene prioridad porque sincroniza al equipo del CIEP), después `release` (publica al endpoint público). |
| **Salida exitosa** | Ambos canales actualizados, ambos logs poblados. |
| **Si falla** | Si `internos` falla, no se ejecuta `release`. Si `release` falla, `internos` ya tuvo éxito (estado parcial documentado). |

**Archivos de configuración asociados.** El script lee credenciales del servidor desde `endpoint-credentials.sh` (gitignored, no versionado). Una plantilla `endpoint-credentials.template.sh` versionada documenta los campos requeridos (`SSH_HOST`, `SSH_USER`, `SSH_REMOTE_PATH`, opcionalmente `CARPETA_INVESTIGADORES_PATH`).
...
**Estado actual del código vs. estado del modelo institucional.** El sub-comando `publicar.sh internos` (ver §3.2) sincroniza el repo de desarrollo a la Carpeta usando `rsync` con lista de exclusiones, no `git pull` desde dentro de la Carpeta. Eso es **modelo incorrecto que funciona accidentalmente** — los excludes de rsync replican aproximadamente el comportamiento que daría un `git pull`, pero introducen tres riesgos:

1. **Pisotea los archivos del clon Git que Git está rastreando.** Si el clon Git tiene HEAD (su versión actual) apuntando a un commit distinto al `master` de desarrollo, rsync sobreescribe archivos sin que Git se entere, dejando el clon con archivos desincronizados respecto a la versión que Git cree que tiene. Detectable solo con `git status` posterior.
2. **Puede borrar trabajo personal de investigadores en `users/`** si el flag `--delete` se aplica sin excludes correctos para `users/`. La versión actual del script tiene excludes que cubren `users/`, pero la fragilidad estructural sigue presente: cualquier ajuste futuro del script puede reintroducir el riesgo.
3. **No usa el mecanismo Git para versiones nuevas.** Si el clon Git tiene etiquetas de versión locales desactualizadas (etiquetas que apuntaban a `v7.x` cuando ahora ya existe `v8.x`), rsync no las actualiza al estado actual del repositorio remoto. El operador puede creer que sincronizó pero las etiquetas del clon siguen apuntando a versiones viejas.

**Por qué este modelo aún no se reescribió.** El descubrimiento de que la Carpeta es un clon Git ocurrió durante la sesión donde se construyó `publicar.sh internos` (mayo 2026). El sub-comando ya estaba implementado con rsync cuando el modelo correcto se entendió. Decisión consciente: documentar el modelo arquitectónico institucional ahora aunque el código aún use el modelo incorrecto, y declarar el rediseño como pendiente formal en §7. La razón para no revertir o eliminar el sub-comando inmediatamente es operativa — el rsync funciona y sirve al equipo, eliminarlo dejaría al equipo sin mecanismo de sincronización mientras se construye el reemplazo. La razón para documentar la disonancia ahora es institucional — el aprendizaje arquitectónico más valioso de la sesión no debe perderse en memoria mientras se espera al rediseño.

**Operación cotidiana del investigador.** En la práctica diaria, el investigador no ejecuta nada para mantener la Carpeta sincronizada con el repo de producción. La sincronización es responsabilidad del investigador principal vía `publicar.sh internos`, y Dropbox propaga el resultado al disco de cada investigador. El investigador puede ejecutar `cd <carpeta>; git log --oneline -1` en cualquier momento para verificar qué versión tiene localmente.
...
| `publicar.sh`                                                                                                  | Implementado parcialmente. Sub-comandos `release` (deploy al endpoint) y `todo` (combinado) funcionan según §3.2. Sub-comando `internos` (sincronización a Carpeta de investigadores) funciona pero usa modelo arquitectónico incorrecto (rsync con excludes en lugar de `git pull` desde el clon, ver §6.7); rediseño documentado abajo.                                                                                                                                                                                                                                   | `release` y `todo`: ya disponibles. `internos`: rediseño pendiente.                                                                                                                |
...
| **Rediseño de `publicar.sh internos` con modelo `git pull`**                                                   | El sub-comando actual sincroniza usando `rsync` desde el repo de desarrollo a la Carpeta de investigadores. El modelo institucional correcto, documentado en §6.7, es `git pull` desde dentro del clon Git que la Carpeta ya es. El rediseño consiste en reescribir `do_internos` para entrar a `CARPETA_INVESTIGADORES_PATH`, ejecutar `git fetch origin && git checkout <tag>`, verificar working tree limpio antes y después, registrar el deploy en `governance/deploys/carpeta-investigadores.log`. Preserva el backup tar.gz pre-operación (sigue siendo red de seguridad). Elimina la lista de exclusiones (deja de ser necesaria con `git pull`).                                            | Sin calendario fijo. El sub-comando actual funciona accidentalmente con rsync; el rediseño urge cuando se introduzca el primer cambio que rompa la equivalencia accidental rsync-vs-git-pull, o cuando un investigador reporte pérdida de archivos personales en `users/`.                            |

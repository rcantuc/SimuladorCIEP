...
Los hallazgos 🟢 de rutas ya fueron **corregidos en esta auditoría** (ver sección 5). Los 🔴 y 🟡 requieren decisión del autor y se detallan abajo.
...
- **Decisión pendiente del autor:** confirmar la lista canónica de componentes y qué cubre cada uno (`resto` y `transferencias` no tienen descripción en ningún lado). Nota: `SIM.do` llama `GastoPC educacion salud pensiones energia resto transferencias`, consistente con el código.
...
- **Decisión pendiente:** documentarlas o marcarlas como internas (parecen ser de uso interno para otros comandos del Simulador).
...
1. **`profile.do` anuncia una opción inexistente:** el menú de bienvenida muestra `DatosAbiertos {claves} [, NOGraphs DESDE(real) MES]` — la opción `MES` no existe en `DatosAbiertos.ado`.
2. **Contradicción de ruta canónica:** `sysprofile.do` apunta a `~/CIEP Dropbox/SimuladorCIEP/` mientras `governance/arquitectura-distribucion.md` §2 documenta `~/Dropbox-CIEP/SimuladorCIEP/`. Una de las dos debe declararse canónica (o documentarse que ambas formas existen según la configuración de Dropbox de cada máquina).
...
1. **Resolver los dos críticos antes de la siguiente release:** `SCN` (opción `aniomax` fantasma) y `GastoPC` (componentes renombrados). Ambos hacen que un usuario nuevo fracase siguiendo la ayuda oficial.
2. **Adoptar la línea `*! version` sincronizada** en `.ado` y `.sthlp` (hallazgo 4.1).
...
4. **Decidir el estatus de las opciones internas** (`pef`/`ppef`/`aprobado` en `PEF`; `pibvp`/`pibvf`/`files` en `DatosAbiertos`): documentarlas o marcarlas explícitamente como de uso interno en un comentario del `.ado`.

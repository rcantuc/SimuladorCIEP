...
Todas las bases de datos de la ENIGH 2020 deben estar en formato *.dta
...
*Año de nacimiento;
gen anac_e=.;
replace anac_e=2020-edad if edad!=.;
...
save "$bases/ic_rezedu20.dta", replace;
...
sort folioviv foliohog numren;
save "$bases/ocupados20.dta", replace;
...
sort folioviv foliohog numren;
merge folioviv foliohog numren using "$bases/ocupados20.dta";
tab _merge;
drop _merge;
...
*A partir de 2020, se reporta la población que respondió estar afiliado o inscrito
al Seguro Popular, o que tiene derecho a los servicios del
Instituto de Salud para el Bienestar (INSABI), lo anterior
de acuerdo con el cuestionario de la ENIGH 2020.
*****************************************************************************;
...
*Acceso reportado;
replace ic_asalud=0 if pop_insabi=="1" | (pop_insabi=="2" & atemed=="1" & (inst_1=="1" |
...
label define discap  0 "Sin presencia de discapacidad"
                     1 "Con presencia de discapacidad";
label value discap discap;

keep folioviv foliohog numren sexo sa_* *_sa pop_insabi atemed inst_* inscr_* segvol_* ic_asalud discap;
sort folioviv foliohog numren;
save "$bases/ic_asalud20.dta", replace;
...
sort folioviv foliohog numren;
save "$bases/prestaciones20.dta", replace;
...
sort  folioviv foliohog numren;
save "$bases/pensiones20.dta", replace;
...
sort  folioviv foliohog numren;
merge  folioviv foliohog numren using "$bases/prestaciones20.dta";
tab _merge;
drop _merge;

sort  folioviv foliohog numren;
merge  folioviv foliohog numren using "$bases/pensiones20.dta";
tab _merge;
drop _merge;
...
scalar	lp1_urb	=	1702.28;
scalar	lp1_rur=	1299.30;
...
keep folioviv foliohog numren tipo_trab* aforlab* smlab*
		   smcv aforecv pea jub ss_dir par jef_ss cony_ss hijo_ss s_salud pam ing_pam
        	ic_segsoc ;
sort folioviv foliohog numren;
save "$bases/ic_segsoc20.dta", replace;
...
save "$bases/ic_cev20.dta", replace;
...
save "$bases/ic_sbv20.dta", replace;
...
save "$bases/menores20.dta", replace;
...
sort folioviv foliohog;
merge folioviv foliohog using "$bases/menores20.dta";
tab _merge;
drop _merge;
...
save "$bases/ic_ali20.dta", replace;
...
replace ing_1=(ing_1/dic19)/12 if clave=="P009" | clave=="P016";
...
sort folioviv foliohog;
save "$bases/ingreso_deflactado20.dta", replace;
...
save "$bases/ingresonomonetario_def20.dta", replace;
...
sort folioviv foliohog;
save "$bases/esp_def20.dta", replace;
use "$bases/ingresonomonetario_def20.dta", clear;
...
sort folioviv foliohog;
save "$bases/reg_def20.dta", replace;
...
merge folioviv foliohog using "$bases/ingreso_deflactado20.dta";
tab _merge;
drop _merge;
...
merge folioviv foliohog using "$bases/esp_def20.dta";
tab _merge;
drop _merge;
...
merge folioviv foliohog using "$bases/reg_def20.dta";
tab _merge;
drop _merge;
...
sort folioviv foliohog;
save "$bases/ingresotot20.dta", replace;
...
sort folioviv foliohog;
save "$bases/tamhogesc20.dta", replace;
...
use "$bases/ingresotot20.dta", clear;

*Incorporación de la información sobre el tamaño del hogar ajustado;

merge folioviv foliohog using "$bases/tamhogesc20.dta";
tab _merge;
drop _merge;
...
scalar lp2_urb = 3559.88;
scalar lp2_rur = 2520.16;
...
keep folioviv foliohog factor tam_loc rururb tamhogesc ict ictpc plp_e plp 
	 ...
	 pago_esp reg_esp; 
sort folioviv foliohog;
save "$bases/p_ingresos20.dta", replace;
...
use "$bases/ic_rezedu20.dta", clear;

merge folioviv foliohog numren using "$bases/ic_asalud20.dta";
tab _merge;
drop _merge;
sort folioviv foliohog numren;

merge folioviv foliohog numren using "$bases/ic_segsoc20.dta";
tab _merge;
drop _merge;
sort folioviv foliohog;

merge folioviv foliohog using "$bases/ic_cev20.dta";
tab _merge;
drop _merge;
sort folioviv foliohog;

merge folioviv foliohog using "$bases/ic_sbv20.dta";
tab _merge;
drop _merge;
sort folioviv foliohog;

merge folioviv foliohog using "$bases/ic_ali20.dta";
tab _merge;
drop _merge;
sort folioviv foliohog;

merge folioviv foliohog using "$bases/p_ingresos20.dta";
tab _merge;
drop _merge;
...
sort folioviv foliohog numren;
save "$bases/pobreza_20.dta", replace;

...
quietly {
		...
		forvalues k=1(1)`=_N' {
			...
			* FROM Nodes *
			...
			if _rc != 0 & profile[`k'] != 0 {
				...
				local nodes `"`nodes'{"name":"`faccountname'"},"'
			...
			* TO Nodes *
			...
			if _rc != 0 & profile[`k'] != 0 {
				...
				local nodes `"`nodes'{"name":"`taccountname'"},"'
			...
			if profile[`k'] != 0 {
				local links `"`links'{"target":"``node`taccountname'''","value":"`=string(profile[`k'],"%20.0f")'","source":"``node`faccountname'''"},"'
			}
		}
	}
	...
	*noisily di in w `"FusionCharts.ready(function()_{var_myChart=new_FusionCharts({type:"sankey",renderAt:"sankey-`name'",width:"100%",height:"100%",dataFormat:"json",dataSource}).render();});});"'
	...
	//}
	//if "`c(os)'" == "Unix" & "`c(username)'" == "root" {
		filefilter `sankey3' `"${SIMROOT}/users/$id/`folder'/sankey-`name'.js"', from(".,") to("0") replace
	//}
}
end
